...
def setup():
    for priority,module in test_modules:
        try:
            ...
            api.current_logger().warning("Error installing mock module: %s, %s", str(e.stderr), semodule.get("stderr", "fuck"))
            continue
...
def teardown():
    ...
    for priority,module in reverse(test_modules + [["400", "permissive_abrt_t"]]):
        try:
