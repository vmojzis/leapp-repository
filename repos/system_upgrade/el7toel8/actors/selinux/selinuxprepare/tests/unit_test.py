...
class run_mocked(object):
    ...
    def __call__(self, args, split=True):
        ...
        if self.args[0] == 'semodule':
            stdout = ["libsemanage.semanage_direct_remove_key: Removing last dummy module (no other dummy module exists at another priority)."]
            self.removed_modules.add(self.args[-1])
        else :
            non_semodule_calls += 1
...
def test_removeCustomModules(monkeypatch):
    ...
    assert len(set(mock_modules) - library.run.removed_modules) == 0